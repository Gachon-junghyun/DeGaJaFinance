...
| **Find the evidence before re-measuring it** | `python -X utf8 -m module_evidence cite <말>` · `where <티커>` · `show <M####>` | This folder holds **1,644 numbered claims** and the only way to reach one was grep. Ask before you re-search: a re-measure of a fact already on file writes the **same fact at a different value**. ⚠ 0 hits is a vocabulary statement, not absence (`D94`). |
| **Allocate ledger IDs** | `python -X utf8 -m module_evidence next-id M --count <n>` (계열 M/D/R/P/C/S) | Replaces the hand-typed *"ID 3-grep at WRITE time"* at the head of every append block. Live-scans `handoff/` + `REPORT/` + `llm_outputs/`, so a stale index cannot hand out someone else's number (`D76` collision class). |
