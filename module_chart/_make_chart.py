...
from ._renderer import plot_combined_chart
from ._metadata import generate_metadata, generate_chart_read
...
def save_text_chart(
    ...
    if include_chart_read:
        # fail-safe: CHART_READ 생성이 어떤 종목/데이터에서 터져도 기존 차트 렌더는 보존
        try:
            read_block = generate_chart_read(df)
        except Exception as exc:  # noqa: BLE001 — 파이프라인 보호가 우선
            read_block = f"(CHART_READ 생성 실패 — 무시하고 차트/메타데이터 사용: {exc!r})"
        body += "\n\n" + "── CHART_READ (결정론 구조판정) " + "─" * 30 + "\n" + read_block
